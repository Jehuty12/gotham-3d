...
export function bindSettings(living, signal) {
  ...
  panel.innerHTML = `<div class="settings-title">ATMOSPHÈRE <button id="settings-close" aria-label="Fermer les réglages">×</button></div>
    ...
    <label>Mission de conduite <select id="vehicle-mission-choice"><option value="">Séquence automatique</option value="drive">Rejoindre</option><option value="follow">Poursuivre une cible</option><option value="evade">Semer la poursuite</option><option value="timed">Trajet chronométré</option><option value="intercept">Intercepter</option><option value="escort">Escorter</option></select></label>
  ...
  panel.querySelector('#vehicle-mission-choice').addEventListener('change',e=>{living.gameplay.vehicleMissions.offeredType=e.target.value||null;},options);
}
