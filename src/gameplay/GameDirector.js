...
export class GameDirector {
  constructor(living,player,signal,{ui=true}={}) {
    ...
    if(ui) {
      ...
      window.addEventListener('mousedown',e=>{if(e.button===0&&player.controls.isLocked&&this.mode==='VIGILANTE'&&!this.health.dead&&!this.city.collisionWorld.domain){this.camera.getWorldDirection(this.direction);this.combat.attack(this.enemies.enemies,this.camera.position,this.direction);}},{signal});
    }
  ...
  setMode(mode) {
    ...
    this.health.hp=100;this.health.dead=false;this.player.physics.frozen=false;this.target=null;this.syncTime=1;
  ...
  tick(dt) {
    ...
    const hidden=Boolean(this.city.collisionWorld.domain);
  ...
  update(dt) {
    ...
    this.target=null;
    if(this.mode==='VIGILANTE'&&!this.health.dead&&!this.city.collisionWorld.domain){
  ...
  snapshot() {
    ...
    return {gameMode:this.mode,activeMission:this.missions.active?.id??null,missionState:this.missions.active?.state??null,availableMissions:this.missions.missions.filter(m=>m.state==='AVAILABLE').length,
      activeCrimes:this.crimes.events.length,health:this.health.hp,movementMode:this.traversal.mode,grappleState:this.vertical.grapple.state,glideState:this.traversal.glide.active,
  ...
  dispose(){this.hud?.dispose();this.player.onAction=this.originalAction;}
}
